...
def BA(posi, nega):
    ...
    # return bas, cnt_ba / len(posi)
    return cnt_ba / len(posi)

def RECALL(posi, nega, k=10):
    num = len(posi)
    mean_recall = 0
    recalls = []
    for p, n in zip(posi, nega):
        y_true, y_score = _canonical(p, n)
        score_idx = np.argsort(y_score)[-k:]
        
        selected_labels = y_true[score_idx]
        recall_cnt = np.count_nonzero(selected_labels) / np.count_nonzero(y_true)
        mean_recall += recall_cnt
        recalls.append(mean_recall)
        # print(score_idx, recall_cnt)

    # return recalls, mean_recall / num
    return mean_recall / num
