...
def getui(uc, co, oi):
    ui_dict = {}
    for user, comb_arr in uc.items():
        ui_dict[user] = set()
        for c in comb_arr:
            if not co.__contains__(c):
                continue
            for o in co[c]:
                for i in oi[o]:
                    ui_dict[user].add(i)
...
class SubgraphCollatorVocab:
    def __init__(self,
        ...
        self.ui = getui(self.uc, self.co, self.oi)
    ...
    def __call__(self, batch):
        ...
        batch_o_ii = self._get_ii(item_emb_index)
        batch_ui = self._get_ui(user_emb_index, item_emb_index)
        batch_oi = self._get_oi(outfit_list, item_arr)

        # print(item_emb_index.size(), user_emb_index.size(), outfit_emb.size(), scene_emb.size())
    ...
    def _get_ui(self, user_emb_index, item_emb_index):
        batch_matrix = torch.zeros((len(user_emb_index), len(item_emb_index)))
        for i, user_id in enumerate(user_emb_index):
            user = self.user_id2vocab[int(user_id.data)]
            for j, item_id in enumerate(item_emb_index):
                item = self.item_id2vocab[int(item_id.data)]
                if self.ui[user].__contains__(item):
                    batch_matrix[i, j] = 1
        return batch_matrix

    def _get_ii(self, item_emb_index):
        batch_matrix = torch.zeros((len(item_emb_index), len(item_emb_index)))
        for i, item_id in enumerate(item_emb_index):
            item1 = self.item_id2vocab[int(item_id.data)]
            outfit_arr = self.io[item1]
            
            for j, item_id in enumerate(item_emb_index):
                item2 = self.item_id2vocab[int(item_id.data)]
                for o in outfit_arr:
                    if self.oi[o].__contains__(item2):
                        batch_matrix[i, j] = 1
                        break
        return batch_matrix
    
    def _get_oi(self, outfits, items):
        batch_matrix = torch.zeros((len(outfits), len(items)))
        for i, o in enumerate(outfits):
            item_arr = self.oi[o]
            for j, item in enumerate(items):
                if item_arr.__contains__(item):
                    batch_matrix[i][j]=1
        return batch_matrix